%% Lotka Volterra Spiralling Solution
% DESCRIPTIVE TEXT
...
for i=1:Niter
     ...
     y(i+1) = dt*(-F*y(i)+G*x(i)*y(i))+y(i);
end

% Backward Euler:
% for i=1:Niter
%      time(i+1)=dt*i
%      x(i+1)=x(i)./(1-dt*(P-B*y(i))
%      y(i+1)=y(i)./(1-dt*(-F+G*x(i))
% end
subplot(1, 2, 1)
plot(time, x)
hold on
...
legend('Prey per unit area', 'Predator per unit area')
xlim([0 50])
subplot(1, 2, 2)
plot(x, y)